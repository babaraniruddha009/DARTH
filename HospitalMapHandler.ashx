<%@ WebHandler Language="VB" Class="HospitalMapHandler" %>
...
Public Class HospitalMapHandler : Implements IHttpHandler
    ...
    Public Sub ProcessRequest(ByVal context As HttpContext) Implements IHttpHandler.ProcessRequest
        ...
        Dim cmd As SqlCommand = New SqlCommand("spGetHospitalNamesCity", con)
        cmd.CommandType = CommandType.StoredProcedure
        cmd.Parameters.AddWithValue("@term", term)
        con.Open()
        Dim rdr As SqlDataReader = cmd.ExecuteReader()
        While (rdr.Read())
            ListLocationNames.Add(rdr("Hospital_Name").ToString())
        End While
